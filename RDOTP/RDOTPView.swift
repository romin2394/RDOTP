...
public class RDOTPView: UIView {
    ...
    public var otpTextFieldActiveBorderWidth: CGFloat = 2
    ...
    var aciveTextField:RDOTPTextField?
    ...
    public func initializeUI() {
        ...
        aciveTextField = (viewWithTag(1) as? RDOTPTextField)
        ...
        self.aciveTextField = viewWithTag(1) as? RDOTPTextField
    ...
    @objc func getResponder(){
        if hasEnteredAllOTP && aciveTextField?.tag != self.otpTextFieldsCount{
            (viewWithTag(self.otpTextFieldsCount) as? RDOTPTextField)?.becomeFirstResponder()
        }else{
            aciveTextField?.becomeFirstResponder()
        }
    ...
    fileprivate func initializeotpTextFields() {
        ...
        for index in stride(from: 0, to: otpTextFieldsCount, by: 1) {
            let oldotpTextField = viewWithTag(index + 1) as? RDOTPTextField
    ...
    fileprivate func getotpTextField(forIndex index: Int) -> RDOTPTextField {
        ...
        let otpTextField = RDOTPTextField(frame: fieldFrame)
    ...
    fileprivate func calculateEnteredOTPSTring(isDeleted: Bool) {
        if isDeleted {
            ...
            for index in stride(from: 0, to: otpTextFieldsCount, by: 1) {
                var otpTextField = viewWithTag(index + 1) as? RDOTPTextField
...
extension RDOTPView: UITextFieldDelegate {
    public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        (textField as? RDOTPTextField)?.shapeLayer.fillColor = otpTextFieldActiveBorderColor.cgColor
        (textField as? RDOTPTextField)?.shapeLayer.strokeColor = otpTextFieldActiveBorderColor.cgColor
        (textField as? RDOTPTextField)?.shapeLayer.lineWidth = otpTextFieldActiveBorderWidth
        self.aciveTextField = textField as? RDOTPTextField
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if replacedText.count >= 1 {
            ...
            (textField as! RDOTPTextField).shapeLayer.fillColor = otpTextFieldDefaultBorderColor.cgColor
            (textField as! RDOTPTextField).shapeLayer.strokeColor = otpTextFieldDefaultBorderColor.cgColor
            (textField as! RDOTPTextField).shapeLayer.lineWidth = otpTextFieldBorderWidth
            
            let nextotpTextField = viewWithTag(textField.tag + 1)
            
            if let nextotpTextField = nextotpTextField as? RDOTPTextField{
            ...
            if textField.tag > 1 && currentText.isEmpty {
                if let prevotpTextField = viewWithTag(textField.tag - 1) as? RDOTPTextField {
